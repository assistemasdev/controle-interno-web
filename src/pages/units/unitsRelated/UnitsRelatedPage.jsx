...
const UnitsRelatedPage = () => {
    ...
    const confirmDelete = async () => {
        try {
            setLoading(true);
            const response = await UnitService.detachOutputUnits(id,{units:[unitToDelete.id]}, navigate);

            setMessage({ type: 'success', text: response.message });
            fetchUnits();
            return;
        } catch (error) {
            setMessage({ type: 'error', text: error.message || 'error ao desassociar unidade' });
            console.error(error);
        } finally {
    ...
    const actions = [
        {
            icon: faTrash,
            title: 'Desassociar Unidade',
            buttonClass: 'btn-danger',
            permission: 'Desassociar unidades de medida',
            ...
            <div className="container-fluid p-1">
                ...
                <div className="form-row mt-4 d-flex justify-content-between align-items-center">
                    <div className="font-weight-bold text-primary text-uppercase mb-1 text-dark d-flex">
                        Lista de Unidades Relacionadas
                    </div>
                    {canAccess('Associar unidades de medida') && (
                        <Button
                        text="Associonar Nova Unidade"
                        className="btn btn-blue-light fw-semibold"
                        link={`/unidades/${id}/relacionadas/criar`}
                        />
                    )}
                </div>
...
export default UnitsRelatedPage;