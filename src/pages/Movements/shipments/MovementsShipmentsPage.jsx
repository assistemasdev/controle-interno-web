...
import useShiptmentFilters from "../../../hooks/filters/useShiptmentFilters";  
import useAction from "../../../hooks/useAction";  
import FilterForm from "../../../components/FilterForm";  
import ListHeader from "../../../components/ListHeader";  
import PageHeader from "../../../components/PageHeader";  

const MovementsShipmentsPage = () => {
    const { id } = useParams();
    ...
    const fetchShipments = useCallback(async (filtersSubmit) => {
        ...
        try {
            ...
            const [
                ...
                fetchAll(entities.movements.shipments.get(id), filtersSubmit || filters),
            ])
    ...
    const { handleFilterSubmit, handleClearFilters, inputsfilters } = useShiptmentFilters(fetchShipments, filters, setFilters);  
    ...
    const actions = useMemo(() => [
        {
            ...
            onClick: (shipment) => navigate(`/movimentos/${id}/carregamentos/${shipment.id}/editar`)
        },
        {
            id: 'details',
            icon: faEye,
            ...
            onClick: (shipment) => navigate(`/movimentos/${id}/carregamentos/${shipment.id}/detalhes/`)
        },
        {
            id: 'delete',
            icon: faTrash,
            ...
            onClick: (movement) => handleDelete(movement, 'Você tem certeza que deseja excluir: ', entities.movements.shipments.delete(id, movement.id), fetchShipments),
        },
        {
            id: 'activate',
            icon: faUndo,
    ...
    return (
        <MainLayout selectedCompany="ALUCOM">
            <PageHeader title="Carregamentos do Movimento" showBackButton={true} backUrl="/movimentos" />
            <div className="container-fluid p-1">
                <FilterForm autoCompleteFields={inputsfilters} onSubmit={handleFilterSubmit} onClear={handleClearFilters} />
                <ListHeader title="Lista de Carregamentos do Movimento" buttonText="Novo Carregamento" buttonLink={`/movimentos/${id}/carregamentos/criar`} canAccess={canAccess} permission="Criar carregamentos" />
                <DynamicTable 
                    headers={headers} 
                    data={shipments} 
                    actions={actions} 
...
export default MovementsShipmentsPage;
